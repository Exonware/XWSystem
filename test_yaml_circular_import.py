#!/usr/bin/env python3
"""
Test: Does yaml circular import happen when yaml IS installed?

This tests if the circular import error occurs even when PyYAML is installed.
"""
...
print("="*60)
print("TEST: Import xwsystem with yaml installed (NO xwlazy)")
print("="*60)
print()

# First check if yaml is installed
try:
    import yaml
    print(f"✅ PyYAML is installed: {yaml.__version__}")
except ImportError:
    print("❌ PyYAML is NOT installed - installing it first...")
    import subprocess
    subprocess.run([sys.executable, "-m", "pip", "install", "PyYAML", "-q"], check=True)
    import yaml
    print(f"✅ PyYAML installed: {yaml.__version__}")

print("\nNow testing xwsystem import...")
print()
...
try:
    ...
    print(f"✅ Success: YAML encoding works")
    
    print("\n" + "="*60)
    print("✅ RESULT: NO CIRCULAR IMPORT ERROR!")
    print("The circular import only happens WITH xwlazy enabled.")
    print("="*60)
